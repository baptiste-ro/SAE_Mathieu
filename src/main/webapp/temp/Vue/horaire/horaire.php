<?php
    ...
    $departure = $content_pushed['departure'];
    $arrival = $content_pushed['arrival'];

    // Initialisation du contenu de la requête
    $request = "SELECT Code_Liaison AS code_liaison FROM liaison WHERE Id_Port = ? AND Id_Port_1 = ?";

    // Création d'une preparedStatement pour exécuter la requête
    $stmt = $conn->prepare($request);
    $stmt->bind_param("ii", $departure, $arrival);
    ...
    //Indique au navigateur que la réponse sera en format JSON
